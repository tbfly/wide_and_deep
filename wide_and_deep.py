...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string("model_dir", "./wnd_model", "Base directory for output models.")
flags.DEFINE_string("model_type", "wide_n_deep", "Valid model types: {'wide', 'deep', 'wide_n_deep'}.")
flags.DEFINE_integer("train_steps", 200, "Number of training steps.")
flags.DEFINE_string("train_data", "./wnd.tr", "Path to the training data.")
flags.DEFINE_string("test_data", "./wnd.tst", "Path to the test data.")
...
def maybe_download():
	...
	train_file.name = train_file_name
	if os.path.isfile(train_file_name):
		...
		urllib.urlretrieve("http://mlr.cs.umass.edu/ml/machine-learning-databases/adult/adult.data", train_file.name) # pylint: disable=line-too-long
		train_file.close()
	...
	if os.path.isfile(test_file_name):
		...
		urllib.urlretrieve("http://mlr.cs.umass.edu/ml/machine-learning-databases/adult/adult.test", test_file.name) # pylint: disable=line-too-long
		test_file.close()
...
def build_estimator(model_dir):
	...
	wide_columns = [gender, native_country, education, occupation, workclass, marital_status,
					relationship, age_buckets, tf.contrib.layers.crossed_column([education, occupation],
					hash_bucket_size=int(1e4)), tf.contrib.layers.crossed_column( [age_buckets, race, occupation],
					hash_bucket_size=int(1e6)), tf.contrib.layers.crossed_column([native_country, occupation],
	...
	deep_columns = [ tf.contrib.layers.embedding_column(workclass, dimension=8),
					...
					tf.contrib.layers.embedding_column(occupation, dimension=8), age, education_num, capital_gain, capital_loss, hours_per_week, ]
